...
import com.switflow.swiftFlow.Request.MaterialSelectionRequest;
import com.switflow.swiftFlow.Response.MaterialSelectionResponse;
import com.switflow.swiftFlow.Service.MaterialSelectionService;
...
public class MaterialSelectionController {
    ...
    @PostMapping("/order/{orderId}/material-selection")
    @PreAuthorize("hasAnyRole('ADMIN','DESIGN','PRODUCTION','MACHINING','INSPECTION')")
    public ResponseEntity<?> saveMaterialSelection(
            @PathVariable long orderId,
            @RequestBody MaterialSelectionRequest request,
        ...
        try {
            ...
            materialSelectionService.saveSelection(orderId, currentRole, selected);
            return ResponseEntity.ok(Map.of("message", "Material selection saved successfully"));
            ...
            return ResponseEntity.status(500).body(Map.of(
                    "message", "Failed to save material selection: " + e.getMessage(),
    ...
    @GetMapping("/order/{orderId}/material-selection")
    @PreAuthorize("hasAnyRole('ADMIN','DESIGN','PRODUCTION','MACHINING','INSPECTION')")
    public ResponseEntity<MaterialSelectionResponse> getMaterialSelection(@PathVariable long orderId) {
        return ResponseEntity.ok(materialSelectionService.getSelection(orderId));
    }

    private Department resolveEffectiveRole(HttpServletRequest req, MaterialSelectionRequest body, String authorityRole) {
